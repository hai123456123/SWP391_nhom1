    ...
    <body>
        <script type="text/javascript">
            ...
            function confirmUnban(userId) {
                if (confirm("Bạn có chắc chắn muốn gỡ cấm tài khoản này?")) {
                                                    ...
                                                    <c:choose>
                                                        <c:when test="${lu.roleId == 2}">
                                                            <tr>
                                                                ...
                                                                <td>Hoạt động</td>
                                                                <td>
                                                                    <form id="banForm_${lu.id}" action="banuser" method="post">
                                                                        <input type="hidden" name="userId" value="${lu.id}">
                                                                            <button type="button" class="btn btn-danger btn-xs" onclick="confirmBan('${lu.id}')">Cấm</button>
                                                                    </form>
                                                                </td>
                                                            </tr>
                                                        </c:when>
                                                        <c:when test="${lu.roleId == 4}">
                                                            <tr>
                                                                <td>${lu.id}</td>
                                                                <td>${lu.fullName}</td>
                                                                <td>${lu.email}</td>
                                                                <td>Đã bị cấm</td>
                                                                <td>
                                                                    <form id="unbanForm_${lu.id}" action="banuser" method="post">
                                                                        <input type="hidden" name="unBanUserId" value="${lu.id}">
                                                                            <button type="button" class="btn btn-success btn-xs" onclick="confirmUnban('${lu.id}')">Bỏ Cấm</button>
                                                                    </form>
                                                                </td>
                                                            </tr>
                                                        </c:when>
