    ...
    throws ServletException, IOException {
        ...
        for (Category category : listCategory) {
            if (category.getName().equalsIgnoreCase(nameCategory)) {
                request.setAttribute("mess", "Tên loại sản phẩm này đã tồn tại!");
                request.getRequestDispatcher("addCategory.jsp").forward(request, response);
            }
        }

        cd.insertCategory(nameCategory);
        List<Category> listCategory1 = cd.getAllCategory();
        request.setAttribute("messaddcategory", "Thêm loại sản phẩm thành công");
        request.setAttribute("listCategory", listCategory1);
